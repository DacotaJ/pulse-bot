            ...
            try:
                ...
                if status == "trial":
                    msg = (f"⏰ <b>До конца бесплатного периода — 3 дня</b>\n\n"
                           ...
                           f"• PRO — 25 000 ₽/мес: мультибизнес + PDF для инвесторов + еженедельный разбор\n\n"
            ...
            try:
                ...
                if status == "trial":
                    msg = ("🔴 <b>Ваш бесплатный период завершён</b>\n\n"
                           ...
                           "• PRO — 25 000 ₽/мес: мультибизнес + PDF для инвесторов + еженедельный разбор\n\n"
...
async def suggest_endpoint(request):
    """Принимает идеи от клиентов из кабинета Mini App"""
    if request.method == "OPTIONS":
    ...
    try:
        ...
        if OWNER_CHAT_ID:
            try:
                bot_app = Application.builder().token(BOT_TOKEN).build()
                msg = f"💡 <b>Новая идея от клиента</b>\n\n👤 {username}\n\n📝 {text}"
                await bot_app.bot.send_message(
                    chat_id=OWNER_CHAT_ID,
                    text=msg,
                    parse_mode="HTML"
                ...
                logging.error(f"Suggest notify error: {e}")
